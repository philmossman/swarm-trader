"""Apex Agent — Aggressive growth analyst, AI infrastructure heavy.
...
def _category_weights(portfolio_weights: dict[str, float]) -> dict[str, float]:
    """Aggregate portfolio weights by Apex category."""
...
def apex_agent(state: AgentState, agent_id: str = "apex_agent"):
    """Apex — aggressive growth analyst with AI infra conviction and contrarian edge."""
    ...
    for ticker in tickers:
        ...
        if cat_key and cat_data:
            ...
            category_label = "Outside Apex Universe"
        ...
        analysis_context = {
            "ticker": ticker,
            "in_apex_universe": cat_key is not None,
...
def _generate_apex_signal(
    ...
    """Generate Apex's investment signal via LLM."""

    template = ChatPromptTemplate.from_messages([
        (
            "system",
            """You are Apex, an aggressive growth investor with deep conviction in AI infrastructure.
...
Be direct. Apex has opinions. Short reasoning (max 150 chars)."""
        ),
        (
            "human",
...
1. Is {ticker} in Apex's target universe? If yes, which category?
...
5. Based on Apex's philosophy, what action makes sense?

Return JSON only:
{{
  "signal": "bullish|bearish|neutral",
  "confidence": <float 0-100>,
  "reasoning": "<direct Apex-style assessment, max 150 chars>"
}}"""
        ),
    ])
